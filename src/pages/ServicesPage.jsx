...
import { updateServicesDraft, clearServicesDraft, setServicesFormSubmitted } from '../redux/slices/formDraftsSlice';
import { submitServiceRequestThunk } from '../redux/slices/data.slice';
import { 
  Wrench, 
  Settings, 
  Clock, 
...
const ServicesPage = () => {
  ...
  // --- QUOTE FORM STATE (REDUX) ---
  const quoteForm = useSelector((state) => state.formDrafts.servicesForm);
  const formSubmitted = useSelector((state) => state.formDrafts.servicesFormSubmitted);

  const handleFormSubmit = async (e) => {
    e.preventDefault();
    if (!quoteForm.name || !quoteForm.phone || !(quoteForm.notes || quoteForm.details)) {
      alert('Please fill out all required fields.');
      return;
    }
    try {
      await dispatch(submitServiceRequestThunk(quoteForm)).unwrap();
      dispatch(setServicesFormSubmitted(true));
    } catch (err) {
  ...
  const servicesSchema = {
    ...
    "@type": "Service",
    "serviceType": "Industrial Concrete Cutting & Core Drilling",
    "provider": {
      "@type": "HardwareStore",
    ...
    "description": "Professional contractor services including heavy diamond core drilling, slab sawing, epoxy anchoring, and expert machinery troubleshooting."
  };

  return (
    <div className={`min-h-screen font-sans transition-colors duration-500 ${isDarkMode ? 'theme-dark' : 'theme-light'}`}>
      <SEO 
        title="Certified Specialty Services | Core Drilling & Concrete Cutting | Hemet Valley Tools"
        description="Explore our specialized industrial concrete services, including heavy core drilling, slab sawing, epoxy anchor grouting, and expert machinery troubleshooting. Request a bid today!"
        keywords="core drilling Hemet, concrete cutting, slab sawing, epoxy anchoring, grouting services, tool calibration, machinery troubleshooting"
        ...
        <section id="quote-form-section" className="border border-zinc-850 p-6 sm:p-10 bg-zinc-950/20 max-w-4xl mx-auto rounded-none text-left">
          ...
          {formSubmitted ? (
            <div className="bg-zinc-900/50 border border-green-500/20 p-8 text-center">
              ...
              <button 
                onClick={() => { dispatch(setServicesFormSubmitted(false)); dispatch(clearServicesDraft()); }}
            ...
            <form onSubmit={handleFormSubmit} className="space-y-6">
              <div className="grid sm:grid-cols-2 gap-6">
                ...
                {/* Name */}
                <div>
                  <label htmlFor="services-name" className="block text-[11px] font-mono tracking-widest text-zinc-500 uppercase mb-2">Your Name *</label>
                  <input 
                    type="text" 
                    id="services-name"
                    required
                    ...
                    onChange={(e) => dispatch(updateServicesDraft({ name: e.target.value }))}
                ...
                {/* Company name */}
                <div>
                  <label htmlFor="services-company" className="block text-[11px] font-mono tracking-widest text-zinc-500 uppercase mb-2">Company / Organization</label>
                  <input 
                    type="text" 
                    id="services-company"
                    value={quoteForm.company || ''}
                    onChange={(e) => dispatch(updateServicesDraft({ company: e.target.value }))}
                ...
                {/* Phone */}
                <div>
                  <label htmlFor="services-phone" className="block text-[11px] font-mono tracking-widest text-zinc-500 uppercase mb-2">Phone Number *</label>
                  <input 
                    type="tel" 
                    id="services-phone"
                    required
                    ...
                    onChange={(e) => dispatch(updateServicesDraft({ phone: e.target.value }))}
                ...
                {/* Trade Select */}
                <div>
                  <label htmlFor="services-trade" className="block text-[11px] font-mono tracking-widest text-zinc-500 uppercase mb-2">Trade / Specialty</label>
                  <select 
                    id="services-trade"
                    value={quoteForm.trade || 'General Contractor'}
                    onChange={(e) => dispatch(updateServicesDraft({ trade: e.target.value }))}
              ...
              {/* Service Category */}
              <div>
                <span className="block text-[11px] font-mono tracking-widest text-zinc-500 uppercase mb-2">Service Segment Needed</span>
                <div role="radiogroup" aria-label="Service Segment Needed" className="grid sm:grid-cols-3 gap-3">
                  {['Equipment Rental', 'Tool / Small Engine Repair', 'Custom Blade & Core Sales'].map((service) => {
                    const isSelected = (quoteForm.serviceNeeded || quoteForm.serviceType || 'Equipment Rental') === service;
                    return (
                      <button
                        key={service}
                        type="button"
                        ...
                        onClick={() => dispatch(updateServicesDraft({ serviceNeeded: service, serviceType: service }))}
              ...
              {/* Requirements notes */}
              <div>
                <label htmlFor="services-notes" className="block text-[11px] font-mono tracking-widest text-zinc-500 uppercase mb-2">Job Specifications &amp; Requirements *</label>
                <textarea 
                  id="services-notes"
                  required
                  aria-required="true"
                  rows="4"
                  value={quoteForm.notes || quoteForm.details || ''}
                  onChange={(e) => dispatch(updateServicesDraft({ notes: e.target.value, details: e.target.value }))}
